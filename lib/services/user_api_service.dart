...
class UserApiService extends BaseApiService {
  Future<User> getCurrentUser() async {
    try {
      ...
      if (!response.data['success']) {
        throw Exception(
            response.data['error']?['message'] ?? 'Failed to get user');
      }
      ...
      if (e.response?.data != null && e.response?.data['error'] != null) {
        throw Exception(
            e.response?.data['error']['message'] ?? 'Failed to get user');
      }
  ...
  Future<List<User>> getFollowers(String userId) async {
    try {
      final response = await dio.get('$baseUrl/users/$userId/followers');
      return (response.data['data']['followers'] as List)
  ...
  Future<List<User>> getFollowing(String userId) async {
    try {
      final response = await dio.get('$baseUrl/users/$userId/following');
      return (response.data['data']['following'] as List)
          .map((u) => User.fromJson(u))
          .toList();
    } catch (e) {
      rethrow;
  ...
  Future<List<Map<String, dynamic>>> getUserPosts(String userId) async {
    try {
      final response = await dio.get('$baseUrl/users/$userId/posts');
      return List<Map<String, dynamic>>.from(response.data['data']['posts']);
    } catch (e) {
      rethrow;
    }
  }
